        ...
        capabilities: [
            {
                ...
                args: [
                    ...
                    { name: 'maxOutputBytes', type: 'number', description: 'Maximum total output size in bytes (stdout + stderr) allowed for provider CLI call' }
                ]
            },
            {
                capability: 'ai.team',
                ...
                args: [
                    ...
                    { name: 'maxOutputBytes', type: 'number', description: 'Maximum total output size in bytes (stdout + stderr) allowed per provider CLI call' }
                ]
            }
        ]
    });
...
export async function executeAiCommand(args: any, externalBudget?: AiExecutionBudget): Promise<any> {
    ...
    if (externalBudget && externalBudget.maxOutputBytes === undefined && args?.maxOutputBytes !== undefined) {
        externalBudget.maxOutputBytes = normalizeBudgetLimit(args?.maxOutputBytes);
    }
...
${instructionResolved}
    ...
    return new Promise((resolve, reject) => {
        ...
        const launch = (spec: AiCliSpec, allowReasoningFallback: boolean) => {
            ...
            const terminateProcess = (reason: string, errorMsg: string) => {
                if (settled) return;
                settled = true;
                budget.budgetExceeded = true;
                budget.exceededReason = reason;
                log(`\n[AI Agent] ${reason}. Terminating provider process...\n`, 'stderr');
                ...
                reject(new Error(`[AI Budget] ${errorMsg}`));
            };

            if (budget.timeoutMs !== undefined && budget.timeoutMs > 0) {
                timeoutTimer = setTimeout(() => {
                    terminateProcess(
                        `Timeout of ${budget.timeoutMs}ms exceeded`,
                        `Provider execution timed out after ${budget.timeoutMs}ms`
                    );
                }, budget.timeoutMs);
            ...
            const processChunk = (chunk: any, stream: 'stdout' | 'stderr') => {
                if (settled) return;
                const chunkBytes = Buffer.isBuffer(chunk) ? chunk.length : Buffer.byteLength(String(chunk), 'utf-8');
                if (budget.maxOutputBytes !== undefined && budget.maxOutputBytes > 0) {
                    if (totalOutputBytes + chunkBytes > budget.maxOutputBytes) {
                        terminateProcess(
                            `Maximum provider output size limit exceeded (${budget.maxOutputBytes} bytes)`,
                            `Provider execution output exceeded limit of ${budget.maxOutputBytes} bytes`
                        );
                        return;
                    }
                }
            ...
            child.stdout.on('data', (d) => processChunk(d, 'stdout'));
            child.stderr.on('data', (d) => processChunk(d, 'stderr'));

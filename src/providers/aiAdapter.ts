...
${instructionResolved}
    ...
    return new Promise((resolve, reject) => {
        ...
        const launch = (spec: AiCliSpec, allowReasoningFallback: boolean) => {
            ...
            const handleOutputData = (d: any, stream: 'stdout' | 'stderr') => {
                if (settled) return;

                const chunkBytes = Buffer.isBuffer(d) ? d.length : Buffer.byteLength(String(d || ''), 'utf-8');
                accumulatedBytes += chunkBytes;

                if (budget.maxOutputBytes !== undefined && accumulatedBytes > budget.maxOutputBytes) {
                    settled = true;
                    cleanupTimers();
                    budget.budgetExceeded = true;
                    budget.exceededReason = `Maximum output size exceeded (${budget.maxOutputBytes} bytes)`;
                    log(`\n[AI Agent] Output limit of ${budget.maxOutputBytes} bytes exceeded. Terminating provider process...\n`, 'stderr');

                    killChildProcess();

                    reject(new Error(`[AI Budget] ${budget.exceededReason}`));
                    return;
                }
            ...
            child.stdout.on('data', (d) => handleOutputData(d, 'stdout'));
            child.stderr.on('data', (d) => handleOutputData(d, 'stderr'));

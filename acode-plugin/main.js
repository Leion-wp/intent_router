...
class IntentRouter {
    ...
    createErrorResponse(code, message, traceId, details = null) {
        const response = {
            success: false,
            data: null,
            error: { code, message, details: details?.message || details },
    ...
    registerDefaultProviders() {
        ...
        this.registerProvider('file', new FileSystemProvider());
    }
}
...
class FileSystemProvider extends BaseProvider {
    constructor() { super('file', 'file'); }
    async canHandle(intent) { return intent.intent.startsWith('file://'); }
    async execute(intent, context) {
        const action = intent.intent.split('://')[1] || 'read';
        const { path, content, recursive } = intent.payload || {};
        if (!path) return this.fail('MISSING_PARAM', 'Path is required');
        
        try {
            const fs = window.fsOperation ? window.fsOperation(path) : null;
            if (!fs) throw new Error('Acode FS API not available');
            ...
            switch (action) {
                ...
                default:
                    return this.fail('UNKNOWN_ACTION', `FS action '${action}' not supported`);
            }
        } catch (e) { return this.fail('FS_ERROR', e.message); }
    }
}
...
async function runTests() {
    ...
    const tests = [
        ...
        { name: 'File List (Root)', intent: { intent: 'file://ls', payload: { path: 'file:///sdcard/' } } }
    ];

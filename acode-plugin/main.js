(function () {
  ...
  class PipelineUI {
    ...
    async loadPipelines() {
      ...
      loadingDiv.textContent = 'Loading pipelines...';
      ...
      if (!projectRoot) {
        ...
        noProjDiv.style.textAlign = 'center';
        noProjDiv.style.padding = '20px';
        noProjDiv.style.color = '#f44336';
        noProjDiv.appendChild(document.createTextNode('No project folder is currently open.'));
        noProjDiv.appendChild(document.createElement('br'));
        noProjDiv.appendChild(document.createElement('br'));
        noProjDiv.appendChild(document.createTextNode('Please open a folder in the sidebar to view its pipelines.'));
        this.$container.appendChild(noProjDiv);
        return;
      }
      ...
      try {
        ...
        if (!exists) {
          const noDirDiv = document.createElement('div');
          noDirDiv.style.padding = '16px';
          noDirDiv.style.background = 'rgba(0,0,0,0.1)';
          noDirDiv.style.borderRadius = '4px';
          noDirDiv.textContent = `No pipeline directory found (${pipelineFolderUrl}).`;
          content.appendChild(noDirDiv);
          return;
        }
        ...
        if (pipelineFiles.length === 0) {
          ...
          noFilesDiv.style.padding = '16px';
          noFilesDiv.style.background = 'rgba(0,0,0,0.1)';
          noFilesDiv.style.borderRadius = '4px';
          noFilesDiv.textContent = 'No *.intent.json files found in pipeline directory.';
          content.appendChild(noFilesDiv);
          return;
        }
        ...
        errDiv.style.background = 'rgba(244,67,54,0.1)';
        errDiv.style.borderRadius = '4px';
        errDiv.textContent = `Error loading pipelines: ${err.message}`;

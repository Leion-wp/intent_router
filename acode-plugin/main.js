(function () {
  ...
  class PipelineRunner {
    ...
    mapIntentToAction(intentName) {
      if (typeof intentName !== 'string' || !intentName.trim()) {
        throw new Error('Missing or invalid intent name');
      }
      const trimmed = intentName.trim();
      const action = trimmed.includes(':') ? trimmed : trimmed.replace(/\./g, ':');
      if (!action.includes(':')) {
        throw new Error(`Malformed intent: '${intentName}' missing separator`);
      }
      const parts = action.split(':');
      if (parts.some((p) => !p.trim())) {
        throw new Error(`Malformed intent: '${intentName}' has empty segment`);
      }
      return action;
    }
    ...
    async runPipelineFromData(pipelineData, onProgress) {
      ...
      for (const step of pipelineData.steps) {
        ...
        let action;
        try {
          action = this.mapIntentToAction(intentName);
        } catch (err) {
          ...
          throw new Error(`Pipeline aborted at step ${stepIndex}: ${err.message}`);
        }

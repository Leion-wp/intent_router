...
class IntentRouter {
    ...
    async detectCapabilities() {
        ...
        try {
            ...
            // Git detection (usually available if terminal is)
        ...
        // Docker detection (Android doesn't have it natively)
    ...
    async execute(intent) {
        ...
        try {
            ...
            this.log('error', `Execution error for ${intent?.intent}`, { code, message });
...
class AIProvider extends BaseProvider {
    ...
    async execute(intent, context) {
        return this.success({ response: "AI Processed: " + (intent.payload?.prompt || "No prompt provided") });
    }
}
...
class DockerProvider extends BaseProvider {
    ...
    async execute(intent, context) {
        ...
        if (intent.payload?.ssh) {
             return this.success(`Executed ${action} on remote docker via SSH (mocked)`, { host: intent.payload.ssh.host });
        }
...
async function runTests() {
    ...
    if (!router) return console.error("Intent Router not initialized");
    ...
    const tests = [
        ...
        { name: 'AI Prompt', intent: { intent: 'ai://prompt', payload: { prompt: 'Hello Rutex' } } },
        { name: 'VSCode Install', intent: { intent: 'vscode://extension/install?id=rutex.intent-router' } },
        { name: 'Terminal Fail (No Command)', intent: { intent: 'terminal://run', payload: {} } },
        { name: 'Docker Fail (Missing Cap)', intent: { intent: 'docker://ps' } },
...
class IntentRouter {
    ...
    async detectCapabilities() {
        ...
        // 3. Docker detection (remote only by default on Android)
        this.capabilities.docker = false; 
    }
    ...
    registerProvider(name, provider) {
        ...
        provider.router = this; // Link router to provider
        this.providers.set(name, provider);
        this.log('info', `Provider registered: ${name}`);
    }
    ...
    normalizeResponse(result, providerName, traceId) {
        ...
        if (!normalized.success) {
            this.log('warn', `Intent failed (${providerName}): ${normalized.error.message}`, normalized.error);
        }
...
class DockerProvider extends BaseProvider {
    ...
    async execute(intent, context) {
        if (context.capabilities.docker) {
             return this.success("Local docker execution (mocked)");
        }
        if (intent.payload?.ssh) {
             return this.success("Remote docker execution via SSH (mocked)");
        }
        return this.fail('CAPABILITY_MISSING', 'Docker is not natively supported on Android. Please provide SSH credentials for remote execution.');
    }
}
...
async function runTests() {
    ...
    const tests = [
        ...
        { name: 'VSCode Mock', intent: { intent: 'vscode://extension/install?id=rutex.intent-router' } },
        { name: 'Terminal Fail (Missing Param)', intent: { intent: 'terminal://run', payload: {} } },
        { name: 'Docker Capability Missing', intent: { intent: 'docker://ps' } },
        { name: 'Invalid Intent Scheme', intent: { intent: 'unknown://action' } }
    ];
...
class DockerProvider extends BaseProvider {
    ...
    async execute(intent, context) {
        ...
        if (intent.payload?.ssh) {
             context.router.log('info', `Routing docker command ${action} to remote host ${intent.payload.ssh.host}`);

  ...
  class IntentRouter {
    ...
    registerAcodeCommands() {
      ...
      const defs = [
        ...
        { name: 'leion.intentRouter.runPipeline', description: 'Intent Router: Run Pipeline', exec: () => this.showPipelineUI() }
      ];
    ...
    async showPipelineUI() {
      try {
        let folderUrl = null;
        if (window.addedFolder && window.addedFolder.length > 0) {
          folderUrl = window.addedFolder[0].url;
        } else if (window.editorManager && editorManager.activeFile && editorManager.activeFile.uri) {
          const parts = String(editorManager.activeFile.uri).split('/');
          parts.pop();
          folderUrl = parts.join('/');
        }

        if (!folderUrl) {
          return this.alert('Error', 'No active folder or file found. Please open a folder first.');
        }

        this.toast('Scanning for pipelines...');

        const fs = this.requireFs();
        const pipelines = [];

        const scanDir = async (dirUrl) => {
          try {
            const items = await fs(dirUrl).lsDir();
            for (const item of items) {
              if (item.isDirectory) {
                // Don't go too deep to avoid freezing, just one or two levels
                if (!item.name.startsWith('.') && item.name !== 'node_modules') {
                  await scanDir(item.url);
                }
              } else if (item.name.endsWith('.intent.json')) {
                pipelines.push({ name: item.name, url: item.url });
              }
            }
          } catch (e) {}
        };

        await scanDir(folderUrl);

        if (pipelines.length === 0) {
          return this.toast('No *.intent.json pipelines found in workspace.');
        }

        const select = this.safeRequire('select');
        if (!select) {
           return this.alert('Error', 'Acode select API is unavailable.');
        }

        const options = pipelines.map(p => [p.url, p.name]);
        const selectedUrl = await select('Select Pipeline to Run', options);
        if (!selectedUrl) return;

        const content = await fs(selectedUrl).readFile('utf-8');
        const pipelineData = JSON.parse(content);

        this.toast(`Running: ${pipelineData.name || 'Pipeline'}`);
        const result = await this.route(pipelineData);

        if (result && result.success) {
           this.toast('Pipeline completed successfully');
        } else {
           this.alert('Pipeline Failed', result?.error || 'Unknown error');
        }

      } catch (error) {
        this.alert('Pipeline Error', error.message);
      }
    }

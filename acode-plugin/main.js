(function () {
  ...
  function decodeBytesToString(uint8Array) {
    ...
    setupCommands() {
      ...
      this.register('network:request', async (data) => {
        ...
        if (data.timeoutMs !== undefined && data.timeoutMs !== null) {
          const num = Number(data.timeoutMs);
          if (isNaN(num) || !Number.isFinite(num) || num <= 0) {
        ...
        try {
          ...
          try {
            ...
            if (controller && controller.signal.aborted) {
              const err = new Error(`Request timed out after ${timeoutMs} ms [request_timeout]`);
          ...
          if (limit !== null && contentLengthHeader) {
            ...
            if (!isNaN(parsedLength) && parsedLength > limit) {
              if (controller) controller.abort();
              const err = new Error(`Response size (${parsedLength} bytes) exceeds limit (${limit} bytes) [response_too_large]`);
              err.code = 'response_too_large';
              err.limit = limit;
              err.size = parsedLength;
              throw err;
            }
          }
          ...
          if (limit !== null && response.body && typeof response.body.getReader === 'function') {
            ...
            try {
              while (true) {
                ...
                if (value) {
                  ...
                  if (accumulatedBytes > limit) {
                    ...
                    if (controller) controller.abort();
                    const err = new Error(`Response size (${accumulatedBytes} bytes) exceeds limit (${limit} bytes) [response_too_large]`);
                    err.code = 'response_too_large';
                    err.limit = limit;
                    err.size = accumulatedBytes;
                    throw err;
                  }
                  chunks.push(value);
                }
              ...
              const rawText = decodeBytesToString(merged);
              body = contentType.includes('application/json') ? JSON.parse(rawText) : rawText;
            } catch (streamErr) {
              try { await reader.cancel(); } catch (_) {}
              if (controller && controller.signal.aborted && streamErr.code !== 'response_too_large') {
                const err = new Error(`Request timed out after ${timeoutMs} ms [request_timeout]`);
                err.code = 'request_timeout';
                err.timeoutMs = timeoutMs;
                throw err;
            ...
            let rawText;
            try {
              rawText = contentType.includes('application/json') && limit === null
                ? await response.json()
                : await response.text();
            } catch (readErr) {
              if (controller && controller.signal.aborted) {

  ...
  class PipelineRunner {
    ...
    async runPipelineFromData(pipelineData, onProgress) {
      ...
      while (currentIndex < steps.length) {
        ...
        if (onProgress) {
          onProgress({ step: currentIndex + 1, total: totalSteps, status: 'running', intent: intentName, stepId: step.id });
        }
        ...
        try {
          const result = await this.router.route({ action, data: payload, id: step.id });
          if (!result.success) {
             throw new Error(result.error || `Step ${currentIndex + 1} failed`);
          }
          logs.push({ step: currentIndex + 1, intent: intentName, success: true, data: result.data, stepId: step.id });
          currentIndex++;
        } catch (err) {
          logs.push({ step: currentIndex + 1, intent: intentName, success: false, error: err.message, stepId: step.id });
          ...
          if (step.onFailure) {
            ...
            if (nextIdx !== -1) {
              ...
              if (onProgress) {
                onProgress({ step: currentIndex + 1, total: totalSteps, status: 'error', error: errorMsg, stepId: step.id });
              }
          ...
          if (onProgress) {
            onProgress({ step: currentIndex + 1, total: totalSteps, status: 'error', error: err.message, stepId: step.id });
          }

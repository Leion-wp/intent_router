...
describe('Acode OpenAI-compatible AI provider bridge (ai:chat)', () => {
  ...
  describe('ai:chat action routing', () => {
    ...
    it('propagates timeoutMs and maxResponseBytes to network:request and preserves error metadata without leaking secrets', async () => {
      const SECRET_KEY = 'sk-budget-secret-key-007';
      router.registerAiProvider('budget-provider', {
        baseUrl: 'https://api.example.com/v1',
        model: 'gpt-4o',
        apiKey: SECRET_KEY
      });
      ...
      router.register('network:request', async (data) => {
        capturedData = data;
        const err = new Error(`Request timed out after ${data.timeoutMs}ms [sk-budget-secret-key-007]`);
      ...
      assert.strictEqual(capturedData.timeoutMs, 5000);
      assert.strictEqual(capturedData.maxResponseBytes, 1048576);
      assert.strictEqual(resTimeout.success, false);
      assert.strictEqual(resTimeout.metadata.code, 'request_timeout');
      assert.strictEqual(resTimeout.error.includes(SECRET_KEY), false);
      assert.ok(resTimeout.error.includes('[REDACTED]'));

      router.register('network:request', async (data) => {
        const err = new Error(`Response size (2048 bytes) exceeds limit (${data.maxResponseBytes} bytes) [response_too_large]`);
      ...
      const resOversized = await router.route({
        action: 'ai:chat',
        data: {
          provider: 'budget-provider',
          messages: [{ role: 'user', content: 'Hello' }],
      ...
      assert.strictEqual(resOversized.success, false);
      assert.strictEqual(resOversized.metadata.code, 'response_too_large');
      assert.strictEqual(resOversized.metadata.limit, 1024);
      assert.strictEqual(resOversized.metadata.size, 2048);
    });
  });
});

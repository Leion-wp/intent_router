...
- **Pipeline Structural Validation**: Pipeline definitions are validated prior to executing any step. Structurally invalid pipelines (e.g. missing intent, invalid payload, duplicate step IDs, or unresolvable `onFailure` references) are rejected fail-fast with an `invalid_pipeline_structure` error before calling any side-effect command handler.

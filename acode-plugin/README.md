...
- **OpenAI-Compatible AI Provider Bridge**: Invoke remote or local/LAN LLMs via `ai:chat` / `ai.chat` using generic runtime provider profiles (`intentRouter.registerAiProvider()`). Secret tokens are kept outside pipeline definitions and automatically redacted from logs and history.
...
intentRouter.registerAiProvider('openrouter', {
  ...
  secret: 'sk-or-v1-your-api-key'
});

// Execute AI chat completions via intent router
intentRouter.route({
  intent: 'ai.chat',
  payload: {
    provider: 'openrouter',
    messages: [
      ...
      { role: 'user', content: 'Write a quick JS function.' }
    ],
    temperature: 0.7
  }
});

// Inspect registered AI provider profiles (non-sensitive metadata)
intentRouter.route({

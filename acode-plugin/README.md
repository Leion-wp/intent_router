...
- **OpenAI-Compatible AI Provider Bridge**: Connect pipelines to remote (OpenRouter, Groq, OpenAI) or local/LAN AI endpoints (Ollama, LM Studio) via `ai:chat` (routable from `ai.chat`). Profiles are registered dynamically at runtime via `intentRouter.registerAiProvider(id, config)` separating secrets from `.intent.json` pipeline definitions. API keys and tokens are automatically redacted from error messages and router logs. Non-sensitive provider profiles can be inspected via `router:ai_providers`.
...
intentRouter.registerAiProvider('openrouter', {
  ...
  secret: 'sk-or-v1-your-secret-key'
});

// Register a secret-less local/LAN AI endpoint (e.g. Ollama)
intentRouter.registerAiProvider('local-ollama', {
  baseUrl: 'http://192.168.1.50:11434/v1',
  model: 'llama3'
});

// Execute AI chat completion (routable via ai:chat action or ai.chat intent)
intentRouter.route({
  intent: 'ai.chat',
  payload: {
    provider: 'openrouter',
    messages: [
      ...
      { role: 'user', content: 'Explain mobile automation.' }
    ],
    temperature: 0.7
  }
});

// Inspect registered AI provider metadata (secrets redacted)
intentRouter.route({

...
- **OpenAI-compatible AI Provider Bridge**: `ai:chat` (routable via `ai.chat`) allows pipelines to invoke remote or local/LAN OpenAI-style `/chat/completions` endpoints. Profiles are registered dynamically using `intentRouter.registerAiProvider(id, profile)` without storing secrets in pipeline JSON files, logs, or run history. Non-sensitive metadata can be inspected via `router:ai_providers`. Network execution budgets (`timeoutMs` and `maxResponseBytes`) specified in `ai:chat` step payloads are propagated directly to `network:request`.
...
// Register an AI provider profile (secret token is never serialized or logged)
...
// Invoke AI chat completion via intent with optional network budget limits
intentRouter.route({
  intent: 'ai.chat',
  payload: {
    provider: 'openrouter',
    messages: [{ role: 'user', content: 'Summarize file content' }],
    temperature: 0.7,
    timeoutMs: 15000,
...
// Inspect registered AI provider profiles (returns non-sensitive metadata only)

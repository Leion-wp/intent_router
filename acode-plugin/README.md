...
- **Fail-Fast Structural Pipeline Validation**: `PipelineRunner.runPipelineFromData` enforces strict structural validation before executing any step handler via `validatePipelineStructure(pipelineData)`. Invalid pipeline objects, non-array steps, invalid intents, non-object payloads, duplicate step IDs, non-boolean `continueOnError`, or dangling `onFailure` references reject immediately prior to the first side effect with code `invalid_pipeline_structure` and a detailed `errors` array.

...
- **Pipeline Structure Fail-Fast Validation**: Before executing any step or side effect, pipelines undergo local pre-execution structural validation via `validatePipelineStructure()`. Structurally invalid pipelines (missing `steps` array, non-object steps, missing/empty `intent`, non-object `payload`, duplicate `id`s, non-boolean `continueOnError`, invalid or non-existent `onFailure` step targets) are rejected before the first handler call with a structured `invalid_pipeline_structure` error.

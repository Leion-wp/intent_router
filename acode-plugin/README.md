...
- **Pipeline Structure Fail-Fast Validation**: Before executing any step in a pipeline, `PipelineRunner` validates the entire document structure via `validatePipelineStructure()`. Malformed steps (missing/empty `intent`, invalid `payload` type, duplicate step `id`s, invalid `continueOnError` boolean, or unresolved `onFailure` step references) are rejected before the first side effect with an `invalid_pipeline_structure` error code.
